...
<template>
    <div class="card-container">
        <Card v-for="(product, index) in products" :key="index">
            ...
            <template #footer>
                <Button label="Add to Cart" class="custom-button" />
            </template>
        </Card>
    </div>
</template>
...
.card-container {
    ...
    width: 60vw;                            /* Takes up the full viewport width */
    height: 80vh;                           /* Takes up the full viewport height */
    padding: 20px;
...
.product-image {
    ...
    height: 100%;           /* Ensures the image fills the card's height */
    object-fit: cover;      /* Ensures the image is not distorted */
}

.p-card-content h3, .p-card-content p  {
    color: black;
}
...
.p-card {
    ...
    transform-style: preserve-3d; /* Keeps 3D child elements */
...
::v-deep .custom-button {
    ...
    cursor: pointer; /* Ensure cursor is a pointer when hovering over the button */
}
